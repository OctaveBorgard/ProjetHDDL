    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, block in enumerate(self.up_blocks):
            if isinstance(block, nn.ConvTranspose2d):
                ...
                hidden_states = F.interpolate(hidden_states, size=skip_connection.shape[-2:], mode="bilinear", align_corners=False)
...
x_any = torch.randn(4, 3, 333, 457)  # Any shape!
output_any = model(x_any)   # → (4, 3, 331, 455)
# %%
n = 100
for i in range(n):
    h = torch.randint(224, 512, (1,)).item()
    w = torch.randint(224, 512, (1,)).item()
    x_any = torch.randn(4, 3, h, w)  # Any shape!
